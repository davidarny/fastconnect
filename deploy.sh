...
# ConnectSafe VPN Deployment Script
# This script deploys the ConnectSafe VPN landing page to a fresh Ubuntu server
...
setup_log_rotation() {
  ...
  cat >/etc/logrotate.d/connectsafe <<EOF
...
show_summary() {
  ...
  echo -e "${GREEN}ConnectSafe VPN has been deployed successfully!${NC}"
  echo ""
...
if [[ "$1" == "-h" || "$1" == "--help" ]]; then
  echo "ConnectSafe VPN Deployment Script"
  echo ""
  echo "Usage: $0 [domain] [email]"
  echo ""
  echo "Arguments:"
  echo "  domain    Domain name for the website (default: connectsafevpn.org)"
  ...
  echo "  - Deploy the ConnectSafe VPN project files"
