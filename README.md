# ConnectSafe VPN Landing Page

A modern, responsive landing page for ConnectSafe VPN with advanced privacy features, dynamic location switching, and AI-powered security. Built with PHP, featuring a sleek design with Tailwind CSS and Alpine.js.
...
2. **Upload to server** using the provided upload script:
   ...
   rsync -av --exclude='.git' --exclude='logs' --exclude='.DS_Store' ./ root@69.62.70.193:/root/connectsafe/
   ```
...
5. **Deploy project files**:
   ...
   ```bash
   mkdir -p /var/www/connectsafe
   # Copy your project files to /var/www/connectsafe
   chown -R www-data:www-data /var/www/connectsafe
   ```
...
```php
$domain = 'https://connectsafevpn.org';  // Your domain
```
...
3. **System Logs**:
   ...
   - Deployment: `/var/log/connectsafe-deploy.log`
...
tail -f /var/www/connectsafe/logs/requests_$(date +%Y-%m-%d).log
```
...
1. **Backup current installation**:
   ...
   ```bash
   cp -r /var/www/connectsafe /var/www/connectsafe.backup.$(date +%Y%m%d)
   ```
...
2. **Upload new files**:
   ...
   rsync -av --exclude='.git' --exclude='logs' ./ root@69.62.70.193:/var/www/connectsafe/
   ```

3. **Set permissions**:
   ```bash
   chown -R www-data:www-data /var/www/connectsafe
   chmod -R 644 /var/www/connectsafe
   find /var/www/connectsafe -type d -exec chmod 755 {} \;
   ```
...
4. **Permission issues**:
   ...
   chown -R www-data:www-data /var/www/connectsafe
   find /var/www/connectsafe -type f -exec chmod 644 {} \;
   find /var/www/connectsafe -type d -exec chmod 755 {} \;
   ```
...
# View API responses
tail -f /var/www/connectsafe/logs/api_responses_$(date +%Y-%m-%d).log
...
1. Check the deployment logs: `/var/log/connectsafe-deploy.log`
...
**ConnectSafe VPN** - Revolutionary VPN technology for the privacy-conscious user.
